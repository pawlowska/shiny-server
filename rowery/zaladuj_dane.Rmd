...
```{r}
plik_pogoda_old="pliki/IMGW_pogoda_20180731.csv"
pogoda_old<-fread(plik_pogoda_old)
pogoda_old[,Data:=as.Date(Data)]
pogoda_new<-czytaj_dane_zipy(od=8, do=8, nazwa_out="pogoda/IMGW_2018_08.csv")
...
```{r}
dane<-wczytaj_z_api(klucz=klucz, od="2014-08-13", do="2018-08-31")
...
```{r}
nazwy<-names(dane_zsumowane)[3:length(names(dane_zsumowane))]
listy_stylow<-zrob_listy_stylow(nazwy, paleta=paleta25_sorted) #w obsluga_sumowania
