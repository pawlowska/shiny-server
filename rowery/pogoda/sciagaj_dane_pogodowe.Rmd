...
```{r}
dni<-lista_dni(od=as.POSIXct("2017-07-01"), do=as.POSIXct("2017-07-22"))
```
...
```{r}
baza=bazaO
temp_min_new<-sciagaj_liste(p_temp_min, dni, baza, zakres='/doba/')
temp_avg_new<-sciagaj_liste(p_temp_avg, dni, baza, zakres='/doba/')
temp_max_new<-sciagaj_liste(p_temp_max, dni, baza, zakres='/doba/')
print(paste(nrow(temp_min_new), nrow(temp_avg_new), nrow(temp_max_new)))
