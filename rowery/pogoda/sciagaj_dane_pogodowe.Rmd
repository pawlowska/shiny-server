...
```{r}
poniedzialki<-lista_poniedzialkow(do=as.POSIXct("2017-07-17"))
```

Alternatively, create a list of days
```{r}
dni<-lista_dni(od=as.POSIXct("2017-06-01"), do=as.POSIXct("2017-07-17"))
```
...
```{r}
baza=bazaO
temp_min_new<-sciagaj_liste(p_temp_min, dni[2:30], baza, zakres='/doba/')
temp_avg_new<-sciagaj_liste(p_temp_avg, dni[2:30], baza, zakres='/doba/')
temp_max_new<-sciagaj_liste(p_temp_max, dni[2:30], baza, zakres='/doba/')
print(paste(nrow(temp_min_new), nrow(temp_avg_new), nrow(temp_max_new)))
...
```{r}
zakres<-149:153
deszcz_new<-sciagaj_liste(p_deszcz_doba, poniedzialki[zakres], bazaHist)
snieg_new<-sciagaj_liste(p_snieg_doba, poniedzialki[zakres], bazaHist)
